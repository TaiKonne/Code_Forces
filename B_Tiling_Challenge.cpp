...
bool check(int x, int y)
{
    for (int i = 0; i < 4; i++)
    {
        int ii = x + dx[i];
        int jj = y + dy[i];
        if (ii >= 1 && jj >= 1 && ii <= n && jj <= n && a[ii][jj] != '.')
        {
            return false;
        }
    }
...
int main()
{
    ...
    memset(a, '#', sizeof(a));
    cin >> n;
    set<char> checkcheckcheckcheckcheckcheckcheckcheck;
    int cnt_dot = 0;
    fr(i, 1, n)
    {
        fr(j, 1, n)
    ...
    for (int i = 1; i <= n; i++)
    {
        for (int j = 1; j <= n; j++)
        {
            checkcheckcheckcheckcheckcheckcheckcheck.insert(a[i][j]);
        }
    }
    if (checkcheckcheckcheckcheckcheckcheckcheck.sz == 1)
        yep;
    else
        nope;
}